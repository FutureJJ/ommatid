...
<div class="cells"><div class="cell"><small>trials</small><b>{len(t_o)} + {len(t_s)}</b></div><div class="cell"><small>real wiring</small><b>{spikes_o:,.0f}</b> sp/20 ms</div><div class="cell"><small>shuffled wiring</small><b>{spikes_s:,.0f}</b> sp/20 ms</div><div class="cell"><small>freeze</small><b>v1</b></div></div></div>
...
<figure class="plate"><header><span class="t"><b>Verdict 2</b> — H2 looming, pre-registered criteria</span> <span class="stamp miss">not established</span></header><div class="body">Using a 5 Hz rise over the 2 s stimulus (a threshold the frozen parameters did not specify), {(lo_o.d_escape>5).mean()*100:.0f} % of loom trials responded, but so did {(t_o[t_o.condition=="disc"].d_escape>5).mean()*100:.0f} % of static-disc controls and {(t_o[t_o.condition=="grey"].d_escape>5).mean()*100:.0f} % of grey trials; gratings 0 %. The pre-registered 200 ms onset criterion was not implemented, because stimulus timing was not frame-locked. The static disc appears abruptly and is itself a looming-like event.</div></figure>
...
<p><b>It suggests</b> that looming reaches DNp04 and DNp02 through the pathway the anatomy predicts (LC4 → DNp04/DNp02), and that the response is weaker when the wiring is shuffled. It does not yet establish that, because the shuffle also changed the input mapping and the criteria were not met as written. Nothing in the model was fitted to behaviour; the pathway was pre-registered, not searched for.</p>
