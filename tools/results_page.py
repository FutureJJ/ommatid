...
<p class="lede">Phase 1, protocol v1: under the pre-registered criteria, none of the three hypotheses is established. An approaching object raised the two escape neurons that looming drives in the animal, and did so less on shuffled wiring — an exploratory result, reported as such, with the confounds listed below.</p>
<p><b>Correction, 11 September 2026, after independent review.</b> An earlier version of this page called the wiring-specificity contrast "met". That was an overstatement: the shuffled-wiring control also changed how camera input is mapped onto neurons (column positions of unlabeled cell types are inferred from the loaded graph), the analysis did not implement the pre-registered 200 ms onset criterion, it pooled controls that behave very differently, and 17 trials contained steps with no camera frame. The numbers below are unchanged; their status is now exploratory. The fixes and the v2 protocol are in the <a href="https://github.com/FutureJJ/ommatid/blob/main/docs/runs.md">run log</a>.</p>
