...
def svg_open(w, h): return [f'<svg viewBox="0 0 {w} {h}" width="100%" role="img" xmlns="http://www.w3.org/2000/svg" font-family="JetBrains Mono, ui-monospace, monospace" font-size="11">']
...
def chart_timecourse(df_o, df_s, pop="DNp04", conds=("loom", "disc", "grey"), bin_ms=100):
    ...
    out = svg_open(W, H)
    # stimulus window
    out.append(f'<rect x="{sx(1000):.1f}" y="{T}" width="{sx(3000)-sx(1000):.1f}" height="{H-T-B}" fill="{INK}" opacity="0.05"/>')
...
def main(orig_dir, shuf_dir):
    ...
    n_excl_o, n_excl_s = int((~t_o_all.valid).sum()), int((~t_s_all.valid).sum())
    di_o = t_o[t_o.condition == "disc"]
    ...
    p_loom_disc = perm_test(lo_o.d_escape, di_o.d_escape)
...
<title>Ommatid — results</title><link rel="stylesheet" href="/style.css?v=6"></head><body><div class="page">
...
<nav class="tabs"><a href="/">Live</a><a class="on" href="/results">Results</a><a href="https://github.com/FutureJJ/ommatid/blob/main/docs/experiment.md">Pre-registration</a><a href="https://github.com/FutureJJ/ommatid/blob/main/docs/runs.md">Run log</a><a href="https://github.com/FutureJJ/ommatid">Source</a></nav>
...
<p><b>Correction, 11 September 2026, after independent review.</b> An earlier version of this page called the wiring-specificity contrast "met". That was an overstatement: the shuffled-wiring control also changed how camera input is mapped onto neurons (column positions of unlabeled cell types are inferred from the loaded graph), the analysis did not implement the pre-registered 200 ms onset criterion, it pooled controls that behave very differently, and 17 trials contained steps with no camera frame. The numbers below are unchanged; their status is now exploratory. A further check the same day showed the escape-neuron response itself was produced by our off-camera surround fill, which darkened the whole eye whenever a dark object entered the camera frame; with a fixed-grey surround the synthetic looming disc no longer drives the escape neurons at all (LC4 2 Hz, DNp04 0 Hz). See the run log, "The surround artefact". The fixes and the v2 protocol are in the <a href="https://github.com/FutureJJ/ommatid/blob/main/docs/runs.md">run log</a>.</p>
<p>Protocol v1, 11 September 2026. 210 trials on the real wiring, 210 on a degree-preserving shuffle of it; {n_excl_o} and {n_excl_s} trials contained steps without a camera frame and are excluded here; seven stimulus conditions on a laptop screen 40 cm in front of the robot's camera (30 cm wide, daylight); body held still; every parameter frozen and published before the first trial (<a href="https://github.com/FutureJJ/ommatid/blob/main/docs/frozen-params.md">frozen-params.md</a>, tag <span style="font-family:var(--mono);font-size:13px">freeze-v1</span>). Whole-network activity: {spikes_o:,.0f} spikes per 20 ms on the real wiring, {spikes_s:,.0f} on the shuffled.</p>

</section>
<div class="verdicts" style="margin-top:28px">
<figure class="plate corner"><header><span class="t"><b>Verdict 1</b> — H2 looming, wiring contrast</span> <span class="stamp miss">exploratory · confounded</span></header><div class="body">Escape/stop descending neurons rose by <b>{lo_o.d_escape.mean():+.1f} Hz</b> during looming on the real wiring versus <b>{lo_s.d_escape.mean():+.1f} Hz</b> on shuffled wiring (permutation p = {pfmt(p_wiring)}). This contrast was not a pre-registered H2 criterion, and the control also altered the input mapping, so it is not evidence of wiring specificity on its own. The rise sits in DNp04 ({dnp04[0]:+.1f} to {dnp04[1]:+.1f} Hz, 95 % CI) and DNp02 ({dnp02[0]:+.1f} to {dnp02[1]:+.1f} Hz), the two direct targets of the LC4 looming detectors; the giant fibre, DNp09 and MDN stayed silent in every trial.</div></figure>
<figure class="plate"><header><span class="t"><b>Verdict 2</b> — H2 looming, pre-registered criteria</span> <span class="stamp miss">not established</span></header><div class="body">Using a 5 Hz rise over the 2 s stimulus (a threshold the frozen parameters did not specify), {(lo_o.d_escape>5).mean()*100:.0f} % of loom trials responded — and so did <b>{(di_o.d_escape>5).mean()*100:.0f} % of static-disc controls</b> (loom vs disc p = {pfmt(p_loom_disc)}); grey {(t_o[t_o.condition=="grey"].d_escape>5).mean()*100:.0f} %, gratings and bright halves 0 %. What the escape neurons respond to in v1 is therefore an abruptly appearing dark object, not expansion as such: the static disc appears in one frame. The pre-registered 200 ms onset criterion could not be tested because stimulus timing was not frame-locked. v2 fades the control disc in and adds a receding disc.</div></figure>
<figure class="plate"><header><span class="t"><b>Verdict 3</b> — H1 optomotor</span> <span class="stamp miss">not met</span></header><div class="body">Steering asymmetry (DNa02 right − left) followed the grating direction in {h1_agree*100:.0f} % of trials (≥ 80 % required) and did not differ from the shuffled graph. The screen covers 41° of a ~300° visual field.</div></figure>
<figure class="plate"><header><span class="t"><b>Verdict 4</b> — H3 phototaxis</span> <span class="stamp miss">not met</span></header><div class="body">Turning toward the bright half in {h3_agree*100:.0f} % of trials; forward-walking neurons (DNa01) never fired in any condition.</div></figure>
</div>

<figure class="plate corner" style="margin-top:28px"><header><span class="t"><b>Plate VII</b> — DNp04, the looming-escape neuron</span> <span>30 trials per condition · 100 ms bins of the fly's time</span></header><div class="body">{chart_timecourse(df_o, df_s, "DNp04")}</div>
...
<figure class="plate"><header><span class="t"><b>Plate VIII</b> — escape/stop rise, every trial</span> <span>stimulus minus own baseline · mean and 95 % CI</span></header><div class="body">{chart_trials(t_o, t_s)}</div>
...
<p><b>It suggests</b> that a dark object appearing in front of the fly reaches DNp04 and DNp02 through the pathway the anatomy predicts (LC4 → DNp04/DNp02), and that the response is weaker when the wiring is shuffled. It does not establish looming selectivity (the static disc evoked the same response), and it does not establish wiring specificity, because the shuffle also changed the input mapping. Nothing in the model was fitted to behaviour; the pathway was pre-registered, not searched for.</p>
...
<footer><span>Connectome: FlyEM male CNS v1.0 · HHMI Janelia, Cambridge, Google Research · CC-BY</span><span>Optic lobe: FlyVis · Turaga lab · MIT</span><span>Neuron model: Shiu et al. 2024</span><span>Code and data: MIT / CC-BY · github.com/FutureJJ/ommatid</span></footer>
</div></body></html>"""
