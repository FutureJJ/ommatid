...
<title>Ommatid — results</title><link rel="stylesheet" href="/style.css"></head><body><div class="page">
...
<figure class="plate corner"><header><b>Verdict 1</b> — H2 looming, wiring specificity <span class="stamp met">met</span></header><div class="body">Escape/stop descending neurons rose by <b>{lo_o.d_escape.mean():+.1f} Hz</b> during looming on the real wiring versus <b>{lo_s.d_escape.mean():+.1f} Hz</b> on shuffled wiring (permutation p = {pfmt(p_wiring)}; criterion p &lt; 0.01). The rise sits in DNp04 ({dnp04[0]:+.1f} to {dnp04[1]:+.1f} Hz, 95 % CI) and DNp02 ({dnp02[0]:+.1f} to {dnp02[1]:+.1f} Hz), the two direct targets of the LC4 looming detectors; the giant fibre, DNp09 and MDN stayed silent in every trial.</div></figure>
<figure class="plate"><header><b>Verdict 2</b> — H2 looming, per-trial thresholds <span class="stamp miss">narrowly missed</span></header><div class="body">A rise &gt; 5 Hz appeared in {(lo_o.d_escape>5).mean()*100:.0f} % of loom trials (≥ 70 % required) and in {(ctl_o.d_escape>5).mean()*100:.0f} % of disc/grating controls (&lt; 20 % required); loom vs controls p {pfmt(p_loom_ctrl_o, True)}. The static-disc control appears abruptly and is itself a looming-like event; v2 fades it in.</div></figure>
<figure class="plate"><header><b>Verdict 3</b> — H1 optomotor <span class="stamp miss">not met</span></header><div class="body">Steering asymmetry (DNa02 right − left) followed the grating direction in {h1_agree*100:.0f} % of trials (≥ 80 % required) and did not differ from the shuffled graph. The screen covers 41° of a ~300° visual field.</div></figure>
<figure class="plate"><header><b>Verdict 4</b> — H3 phototaxis <span class="stamp miss">not met</span></header><div class="body">Turning toward the bright half in {h3_agree*100:.0f} % of trials; forward-walking neurons (DNa01) never fired in any condition.</div></figure>
</div>

<figure class="plate corner" style="margin-top:28px"><header><b>Plate VII</b> — DNp04, the looming-escape neuron <span>30 trials per condition · 100 ms bins of the fly's time</span></header><div class="body">{chart_timecourse(df_o, df_s, "DNp04")}</div>
...
<figure class="plate"><header><b>Plate VIII</b> — escape/stop rise, every trial <span>stimulus minus own baseline · mean and 95 % CI</span></header><div class="body">{chart_trials(t_o, t_s)}</div>
...
<figure class="plate"><header><b>Plate IX</b> — steering asymmetry, every trial <span>DNa02 right − left</span></header><div class="body">{chart_trials(t_o, t_s, "d_turn", "steering asymmetry DNa02 right − left, Hz")}</div>
